...
import com.smhrd.project.service.DBCampusMysqlService;

@RequestMapping("dbcampus")
@RestController
@Qualifier("campusMysqlDataSource")
public class CampusMysqlController {

	@Autowired
	DBCampusMysqlService service;
	...
	@RequestMapping(value = "/createCampusMysqlDb", method = { RequestMethod.POST })
	public String createDb(@RequestBody RequestBodyClass requestBody) {
		System.out.println("[DBController : signupNew]");
		// DB생성 부분
		service.createCampusMysqlDb(requestBody.getReqId(), requestBody.getReqPw());
		return "success";
	}
}

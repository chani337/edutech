...
import com.smhrd.project.service.CGIMysqlService;

@RequestMapping("dbcgi")
@RestController
@Qualifier("cgiMysqlDataSource")
public class CGIMysqlController {

	@Autowired
	CGIMysqlService service;
	...
	@RequestMapping(value = "/createCgiMysqlDb", method = { RequestMethod.POST })
	public String createDb(@RequestBody RequestBodyClass requestBody) {
		System.out.println("[DBController : signupNew]");

		// DB생성 부분
		service.createCgiMysqlDb(requestBody.getReqId(), requestBody.getReqPw());
		return "success";
	}
}

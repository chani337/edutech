...
import com.smhrd.project.service.DBCampusMariaService;

@RequestMapping("dbcampus")
@RestController
@Qualifier("campusMariaDataSource")
public class CampusMariaController {

	@Autowired
	DBCampusMariaService service;
	...
	@RequestMapping(value = "/createCampusMariaDb", method = { RequestMethod.POST })
	public String createDb(@RequestBody RequestBodyClass requestBody) {
		System.out.println("[DBController : signupNew]");
		// DB생성 부분
		service.createCampusMariaDb(requestBody.getReqId(), requestBody.getReqPw());
